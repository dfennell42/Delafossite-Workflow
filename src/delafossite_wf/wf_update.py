...
def get_pkg(suffix):
    ...
    cp = sp.run(['gh','api','-H','accept:application/vnd.github.v3.raw','repos/dfennell42/Delafossite-Workflow/contents/dist/','--jq','.[].name'],capture_output=True)
...
def check_vrsn(suffix):
    ...
    if current_vrsn == new_vrsn:
        ...
        sp.check_call(['gh','cp','dfennell42/Delafossite-Workflow',f'dist/{latest_file}',f'{destpath}'])
        if suffix == '.whl':
            try:
